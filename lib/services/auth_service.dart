  ...
  Future<Map<String, dynamic>> signUp({
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        if (data['status'] == 'success') {
          // Save token and user data - Firebase uses 'id' not '_id'
          _token = data['token'];
          _userId = data['data']['user']['id'] ?? ''; // Firebase uses 'id'
          _userEmail = data['data']['user']['email'] ?? email;
          _userName = data['data']['user']['name'] ?? name;
          _isAuthenticated = true;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'success') {
          ...
          _userId = data['data']['user']['id'] ?? ''; // Firebase uses 'id'
          _userEmail = data['data']['user']['email'] ?? email;
          _userName = data['data']['user']['name'] ?? 'User';
          _isAuthenticated = true;
